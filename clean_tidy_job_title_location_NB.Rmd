...
jobs_raw <- read_csv(csv_link, na = c("", "NA", "NaN")) |>
...
```{r}
companies <- jobs_loc |> select(company) |> unique()
...
```{r}
jobs_onsite <- jobs_loc  |>
  mutate(onsite_remote = as.factor(onsite_remote))

levels(jobs_onsite$onsite_remote)
```
...
```{r}
jobs_cleaned <- jobs_onsite |> 
  select(-c(starts_with("location"), starts_with("analyst"), title, state.abb, posted_date, link))
```

## Potential Analyses

```{r}
jobs_analysis <- jobs_cleaned |> 
  drop_na(salary)
```
