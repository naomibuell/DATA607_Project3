...
#graph
ggplot(tidy_sql_py_r, aes(x = sql_py_r, y = ((sal_low + sal_high) / 2))) + 
  geom_boxplot() 
```

### Compare Bachelor, Master, PHD, Doctorate salaries
...
edu_level <- salary_exist_02 |>
  mutate(
    edu_skill = paste(str_extract(skills, "bachelors "),",", str_extract(skills, "masters "), ",", str_extract(skills, "phd "), ",", str_extract(skills, "doctorate ")),
    .after = skills
  )

#widen df
wide_edu <- edu_level |>
  separate_wider_delim(edu_skill, delim = " , ", names = c("bachelor","master","phd","doctorate"))

#pivot the data frame longer
tidy_edu <- wide_edu |>
  pivot_longer(
    cols = c(bachelor,master,phd,doctorate),
    names_to = "name_drop",
    values_to = "education"
  ) |>
      mutate(
...
ggplot(tidy_edu, aes(x = education, y = sal_low)) + 
  geom_boxplot(aes(x = education, y = sal_low), color = "blue", alpha = .2, fill="lightblue") + 
  geom_boxplot(aes(x = education, y = sal_high), color = "red", alpha = .2, fill="pink") 
```
